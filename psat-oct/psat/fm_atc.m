...
[busidx,lineidx] = findantennas_line(Line);
...
while 1
  ...
  Supply = pgset_supply(Supply);
  Demand = pqset_demand(Demand,tgphi);
  ...
  switch OPF.type
   ...
   case 4  % (N-1) contingency analysis
	...
	a = fm_cpf('atc');
	if ~isempty(a)
	  lcrit = [lcrit; [(a*totp_demand(Demand)+totp_pq(PQ))*MVA, i]];
	else
	  lcrit = [lcrit; [NaN, 0]];
	end
   ...
   case 5  % sensitivity analysis: ranking (dPij/dlambda) & (Pij)
    ...
    PQ = pqmul_pq(PQ,'all',1+lambda);
    pqsum_demand(Demand,1+lambda);

    PV = pvmul_pv(PV,'all',1+lambda+kg);
    pgsum_supply(Supply,1+lambda+kg);

    PV = setvg_pv(PV,'all',OPF.yc(PV.vbus));
    pg = SW.pg;
    SW = setvg_sw(SW,'all',OPF.yc(SW.vbus));
    DAE.y = OPF.yc;
    fm_spf
    fm_disp
