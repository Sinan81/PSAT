...
if OPF.enflow
  Iijmax = getflowmax_line(Line,flow).^2;
else
...
if ~isempty(a),
  ...
  for i = 1:length(a)
    fm_disp(['     Bus #', fvar(getidx_bus(Bus,a(i)),4), ...
             ' -> ',fvar(Vbus(a(i)),8), ...
             ' > ', fvar(Vmax(a(i)),8)],1)
  end
...
if ~isempty(b),
  ...
  for i = 1:length(b)
    fm_disp(['     Bus #', fvar(getidx_bus(Bus,b(i)),4), ...
             ' -> ',fvar(Vbus(b(i)),8), ...
             ' < ', fvar(Vmin(b(i)),8)],1)
  end
  fm_disp('Optimization routine interrupted.',1)
...
while 1
  ...
  Gycall_line(Line)
  Gycall_shunt(Shunt)
  [Iij,Jij,Hij,Iji,Jji,Hji] = fjh2_line(Line,flow,mu_Iijmax,mu_Ijimax);
