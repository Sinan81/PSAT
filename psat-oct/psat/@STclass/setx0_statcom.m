...
for i = 1:a.n
  ...
  if ~isempty(idxg)
    warn_statcom(a,i,[' STATCOM cannot be connected at the same bus as ' ...
  ...
  if a.u(i)
    ...
    PV = remove_pv(PV,idx);
    if isempty(idx)
      warn_statcom(a,i,' no PV generator found at the bus.')
    end
  end
end
DAE.x(a.ist) = a.u.*Bus.Qg(a.bus)./V;
idx = find(DAE.x(a.ist) > ist_max);
if idx, warn_statcom(a,idx,' Ish is over its max limit.'), end
idx = find(DAE.x(a.ist) < ist_min);
if idx, warn_statcom(a,idx,' Ish is under its min limit.'), end
