...
idr = DAE.x(a.idr);
iqr = DAE.x(a.iqr);
vw = DAE.x(getidx_wind(Wind,a.wind));
...
Vw = vw.*Vwrate;
dPwdx = windpower_dfig(a,rho,Vw,A,R,~a.u+omega_m,theta_p,2)./Settings.mva/1e6;
Pw = ng.*windpower_dfig(a,rho,Vw,A,R,~a.u+omega_m,theta_p,1)/Settings.mva/1e6;
Tm = Pw.*iomega;
iqrsign = ones(a.n,1);
w21 = 2*omega_m-1;
...
DAE.Fx = DAE.Fx + sparse(a.omega_m,getidx_wind(Wind,a.wind),Vwrate.*ng.*dPwdx(:,2).*i2Hm.*iomega,DAE.n,DAE.n);
