...
for i = 1:a.n
  ...
  if iqr > a.dat(i,8)
    warn_dfig(a,i,' iqr is over its max limit.')
    check = 0;
  end
  if iqr < a.dat(i,9)
    warn_dfig(a,i,' iqr is under its min limit.')
    check = 0;
  end
  if idr > a.dat(i,10)
    warn_dfig(a,i,' idr is over its max limit.')
    check = 0;
  end
  if idr < a.dat(i,11)
    warn_dfig(a,i,' idr is under its min limit.')
    check = 0;
  end
  ...
  while abs(eqnvw) > 1e-7
    ...
    eqnvw = windpower_dfig(a,rho(i),vw,AA,R,omega,theta,1)-Pw;
    jacvw = windpower_dfig(a,rho(i),vw,AA,R,omega,theta,2);
    incvw = -eqnvw/jacvw(2);
    vw = vw + incvw;
    iter = iter + 1;
  end
