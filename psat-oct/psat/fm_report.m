...
if ishandle(Fig.stat)
  ...
  switch string
   case '1n'
    ...
    [buss,ordbus] = sort(getidx_bus(Bus,0));
   case 'az'
    [buss,ordbus] = sort(Bus.names(1:end));
   otherwise
...
if strcmp(violations,'on')
  ...
  buses = getzeros_bus(Bus);
  buses(getbus_pv(PV)) = 1;
  buses(getbus_sw(SW)) = 1;
...
ntraf = transfno_line(Line) + Ltc.n + Phs.n;
...
if strcmp(violations,'on') && Line.n
  ...
  Ps = abs(P_s(1:Line.n));
  Pr = abs(P_r(1:Line.n));
  Imax = getflowmax_line(Line,'imax');
  Pmax = getflowmax_line(Line,'pmax');
  Smax = getflowmax_line(Line,'smax');
  vIs = Is > Imax;
  vPs = Ps > Pmax;
  vSs = Ss > Smax;
  vIr = Ir > Imax;
