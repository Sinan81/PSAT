...
% Supply parameters
[Csa,Csb,Csc,Dsa,Dsb,Dsc] = costs_supply(Supply);
[Psmax,Psmin] = plim_supply(Supply);
KTBS = tiebreaks_supply(Supply);
ksu = getgamma_supply(Supply);

% Demand parameters
[Cda,Cdb,Cdc,Dda,Ddb,Ddc] = costs_demand(Demand);
[Pdmax,Pdmin] = plim_demand(Demand);
KTBD = tiebreaks_demand(Demand);
qonp = tanphi_demand(Demand);

% Reserve parameters
Cr = costs_rsrv(Rsrv);
[Prmax,Prmin] = plim_rsrv(Rsrv);
DPr = [];
Pr = [];
...
if OPF.envolt
  ...
  Vmin = OPF.vmin*getones_bus(Bus);
  Vmax = OPF.vmax*getones_bus(Bus);
end
Vcmin = Vmin;
Vcmax = Vmax;

if OPF.enflow
  Iijmax = getflowmax_line(Line,OPF.flow).^2;
else
...
[Iij,Iji] = flows_line(Line, OPF.flow);
Iijc = Iij;
Ijic = Iji;
a = [];
b = [];
...
while 1
  ...
  if OPF.line, Line = setstatus_line(Line,OPF.line,0); end

  Line = gcall_line(Line);
  gcall_shunt(Shunt)
  glambda_sw(SW,1+lc,kg);
  glambda_pv(PV,1+lc,kg);
  glambda_pq(PQ,1+lc);
  ...
  %DAE.Gy(:,SW.refbus) = 0;
  Gyc = DAE.Gy;
  [Iijc,Jijc,Hijc,Ijic,Jjic,Hjic] = fjh2_line(Line,OPF.flow,mu_Iijcmax,mu_Ijicmax);
  Hess_c = -hessian_line(Line,ro(n2+1:n4))+Hijc+Hjic-hessian_shunt(Shunt,ro(n2+1:n4));
  ...
  if OPF.line, Line = setstatus_line(Line,OPF.line,status); end
  ...
  %DAE.Gy(:,SW.refbus) = 0;
  %fm_setgy(SW.refbus)
  [Iij,Jij,Hij,Iji,Jji,Hji] = fjh2_line(Line,OPF.flow,mu_Iijmax,mu_Ijimax);
  Hess = -hessian_line(Line,ro(1:n2))+Hij+Hji-hessian_shunt(Shunt,ro(1:n2));
