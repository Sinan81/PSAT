...
if idx, warn_upfc(a,idx,' Vp is over its max limit.'), end
idx = find(DAE.x(a.vp) < vp_min);
if idx, warn_upfc(a,idx,' Vp is under its min limit.'), end

idx = find(DAE.x(a.vq) > vq_max);
if idx, warn_upfc(a,idx,' Vq is over its max limit.'), end
idx = find(DAE.x(a.vq) < vq_min);
if idx, warn_upfc(a,idx,' Vq is under its min limit.'), end

idx = find(DAE.x(a.iq) > iq_max);
if idx, warn_upfc(a,idx,' Ish is over its max limit.'), end
idx = find(DAE.x(a.iq) < iq_min);
if idx, warn_upfc(a,idx,' Ish is under its min limit.'), end
...
for i = 1:a.n
  idxg = findbus_syn(Syn,a.bus1(i));
  if ~isempty(idxg)
    warn_upfc(a,i,[' UPFC cannot be connected at the same bus as ' ...
                'synchronous machines.'])
    continue
  end
  if a.u(i)
    idx = findbus_pv(PV,a.bus1(i));
    PV = remove_pv(PV,idx);
    if isempty(idx)
      warn_upfc(a,i,' no PV generator found at the bus.')
    end
  end
end
