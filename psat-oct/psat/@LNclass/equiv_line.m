function [borderbus,gengroups,yi,y0,zthvec] = equiv_line(a,fid)
...
Ybus = Ybus + ybus_shunt(Shunt,buslist);
Ybus = Ybus + ybus_pq(PQ,buslist);
...
switch EQUIV.equivalent_method
 ...
 case 1 % Thevenin 
  ...
  Ybus = Ybus + ybus_syn(Syn,buslist);
 ...
 case 2 % REI
  ...
  for h = 1:nborder
    ...
    for hh = 1:nisland
      ...
      if ~isempty(jj)
        ...
        gdx = [];
        for u = 1:length(kk)
          sdx = findbus_syn(Syn,kk(u));
          % sdx = [sdx; findbus_pv(PV, kk(u))];
          % sdx = [sdx; findbus_sw(SW, kk(u))];
          if ~isempty(sdx)
            udx = [udx; u];
            gdx = [gdx; sdx];
          end
  ...
  shdata(:,3) = getkv_bus(Bus,borderbus,1);
  shdata(:,4) = 50;
  shdata(:,5) = -real(y0);
  % shdata(:,6) = -imag(y0);
  shdata(:,7) = 1;
...
lndata(:,4) = getkv_bus(Bus,borderbus,1);
lndata(:,5) = 50;
lndata(:,8) = real(zth);
lndata(:,9) = imag(zth);
lndata(:,16) = 1;
