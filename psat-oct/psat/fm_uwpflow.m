 ...
 case 'uwrun'
  ...
  if copt || Copt || Bopt || Hopt || vopt
    ...
    for i = 1:Bus.n
      ...
      idxSu = findbus_supply(Supply,i);
      idxDe = findbus_demand(Demand,i);
      ...
      if Supply.n
        DPg = getpg_supply(Supply,idxSu);
      else % if no Supply, use base case powers
        DPg = getpg_sw(SW,idxSw)+getpg_pv(PV,idxPv);
      end

      % Load Power Directions (Pnl,Qnl)
      if Demand.n
        [Pnl,Qnl] = pqdir_demand(Demand,idxDe);
      else % if no Demand, use base case powers
        [Pnl,Qnl] = pqdir_pq(PQ,idxPq);
      end

      count = fprintf(fid,linef,getidx_bus(Bus,i),0,DPg,Pnl,Qnl,0, ...
                      0,Vmax(i),Vmin(i));
    end
    count = fclose(fid);
  end
