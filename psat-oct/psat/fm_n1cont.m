...
[antennas,fromto] = findantennas_line(Line);
...
[Fijbc,Fjibc] = flows_line(Line,'active');
Pbase = max(Fijbc,Fjibc);
[Fijbc,Fjibc] = flows_line(Line,'apparent');
...
for i = 1:Line.n
  ...
  if ~isempty(idx)
    ...
    idx_sw = findbus_sw(SW,antennas(idx));
    idx_pv = findbus_pv(PV,antennas(idx));
    idx_pq = findbus_pq(PQ,antennas(idx));
    idx_de = findbus_demand(Demand,antennas(idx));
    idx_su = findbus_supply(Supply,antennas(idx));

    SW = remove_sw(SW,idx_sw);
    if ~isempty(idx_sw), SW = add_sw(SW,move2sw_pv(PV)); end
    PV = remove_pv(PV,idx_pv);
    PQ = remove_pq(PQ,idx_pq,'force');
    Demand = remove_demand(Demand,idx_de);
    Supply = remove_supply(Supply,idx_su);
  ...
  if ~isempty(a) && ~isempty(DAE.y) && abs(a-CPF.linit) > 1e-4
    ...
    [Fij,Fji] = flows_line(Line,'active');
    ...
    [Fij,Fji] = flows_line(Line,'apparent');
    % take out the power flow in the line with contingency
    Fij(i) = NaN;
    Fji(i) = NaN;
    Fij(find(abs(Fij) < 1e-6)) = NaN;
...
for i = 1:Line.n
  Rows{2,1}{i,1} = [num2str(getidx_bus(Bus,Line.fr(i))),'-', ...
                    num2str(getidx_bus(Bus,Line.to(i)))];
  Rows{2,1}{i,3} = [num2str(getidx_bus(Bus,Line.fr(Pidx(i)))),'-', ...
                    num2str(getidx_bus(Bus,Line.to(Pidx(i))))];
  if lcrit(i,2)
