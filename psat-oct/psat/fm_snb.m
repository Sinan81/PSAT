...
if ~distr, SW = setgamma_sw(SW); end
...
if distr, Gkcall_supply(Supply); end

if noSup
  Glcall_pv(PV);
  if distr, Gkcall_pv(PV); end
  Glcall_sw(SW);
end
Gkcall_sw(SW);

if SW.n, DAE.Gl(getbus_sw(SW,'v')) = 0; end
if PV.n, DAE.Gl(getbus_pv(PV,'v')) = 0; end
...
while Settings.error > tol
  ...
  glambda_supply(Supply,lambda,distr*kg);
  %Glcall_supply(Supply);
  %if distr, Gkcall_supply(Supply); end

  if noSup
    glambda_pv(PV,lambda,distr*kg)
  ...
  AC = [[hessian_line(Line,M),Ncol;Nrow],[DAE.Gy,DAE.Gk; Kjac]',[Ncol;0]; ...
...
if noSup
  d2 = getnum_sw(SW) + getnum_pv(PV);
  Sbus = double([getbus_pv(PV);getbus_sw(SW)]);
end
