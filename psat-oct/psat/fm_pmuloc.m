...
tic
  ...
  for i = 1:Bus.n
    a = find(nodi == getidx_bus(Bus,i));
    n_link(i,:) = [getidx_bus(Bus,i),length(a)];
  end
  [y,i] = sort(n_link(:,2));
  n_link = n_link(i,:);
  ...
  switch type
   ...
   case 5
    ...
    for i = 1:length(coll1_idx)
      ...
      pmu_test = [pmu_test, getidx_bus(Bus,pmu_idx)];
      if ishandle(Fig.pmu)
    ...
    while 1
      ...
      if basta,
        ...
        pesi(pmu_idx(b)) = 100;
        pmu_test = [pmu_test,getidx_bus(Bus,pmu_idx(b))];
        if ishandle(Fig.pmu)
          set(hdl_pmu,'String',int2str(length(pmu_test)));
          set(hdl_nob,'String',int2str(length(find(pesi == 0))));
          drawnow
    ...
    while 1
      ...
      pmu_test = [pmu_test,getidx_bus(Bus,a(1))];
      pesi(a(1)) = 100;
      pmu_idx2 = find(A(a(1),:));
      pesi(pmu_idx2) = pesi(pmu_idx2) + ones(1,length(pmu_idx2));
      if ishandle(Fig.pmu)
   ...
   case 6
    ...
    for i = 1:Bus.n
      bus2 = bus20;
      bus2(i) = 1;
      pmu_test = getidx_bus(Bus,i);
      bus_pmu = getidx_bus(Bus,i);
      ...
      pmu_test(nodi_el) = [];
      for jjj = 1:length(pmu_test)
        a = length(busidx_line(Line,pmu_test(jjj)));
        if a == 1, antenne = [antenne, jjj]; end
      end
      pmu_test(antenne) = [];
      for jjj = 1:Bus.n
        if isempty(find(pmu_test == getidx_bus(Bus,jjj)))
          a = busidx_line(Line,getidx_bus(Bus,jjj));
          if length(a) > 1
            a_idx = [];
            for kkk = 1:length(a)
    ...
    while 1
      ...
      for i = 1:length(coll_idx)
        ...
        if length(a) >= 1 && length(b) < round(2*j/3)
          ...
          pmu_test = [pmu_test,getidx_bus(Bus,coll_idx(i))];
    ...
    while 1
      ...
      for i = length(a):-1:1
        ...
        if length(find(pesi(coll_idx) > 99)) < 2 && length(coll_idx) > 1
          ...
          pmu_test = [pmu_test,getidx_bus(Bus,a(i))];
          stoppa = 0;
          break
        end
      end
  ...
  for i = 1:Bus.n
    ...
    if iscell(pmu_test)
      pmuyes = find(pmu_test{1} == getidx_bus(Bus,i));
    else
      pmuyes = find(pmu_test == getidx_bus(Bus,i));
    end
    pmuname = Bus.names{i};
    if ~isempty(pmuyes)
      a = [int2str(pmuyes(1)),'      '];
