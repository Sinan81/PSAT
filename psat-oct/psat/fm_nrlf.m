...
while check
  ...
  if initV
    DAE.y(Bus.v) = 1.05*getones_bus(Bus);
    ...
    if Syn.n && ~SW.n
      ...
      if check == 2
        ...
        DAE.y(Bus.a) = mean(getdelta_syn(Syn))*getones_bus(Bus);
        DAE.y(getbus_syn(Syn)) = approxdelta_syn(Syn);
        %check = check + 1;
        check = 0;
      end
      ...
      DAE.y(Bus.a) = DAE.y(SW.refbus(1))*getones_bus(Bus);
      check = 0;
    end
  end
