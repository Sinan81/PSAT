...
if ty1
  ...
  vr = min(vr,vrmax(ty1));
  vr = max(vr,vrmin(ty1));
  DAE.f(p.vf(ty1)) = p.u(ty1).*(-vf+vr-ceiling_exc(p,vf,A(ty1),B(ty1),1))./Te(ty1);
...
if ty2
  ...
  DAE.f(p.vf(ty2)) = p.u(ty2).*(vr1-Ke.*vf-ceiling_exc(p,vf,A(ty2),B(ty2),1))./Te(ty2);
