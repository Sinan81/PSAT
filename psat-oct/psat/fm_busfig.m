...
switch flag
 ...
 case 'area'
  ...
  for i = 1:length(value)
    idx = [idx; find(getarea_bus(Bus,0,0) == value(i))];
  end
 ...
 case 'region'
  ...
  idx = [];
  for i = 1:length(value)
    idx = [idx; find(getregion_bus(Bus,0,0) == value(i))];
  end
  set(hdl_bus,'Value',unique(idx),'ListboxTop',idx(1));
  set(hdl_vol,'Value',[])
  return
 ...
 case 'voltage'
  ...
  idx = [];
  for i = 1:length(value)
    idx = [idx; find(getkv_bus(Bus,0,0) == str2num(voltages{value(i)}))];
  end
  set(hdl_bus,'Value',unique(idx),'ListboxTop',idx(1));
  set(hdl_area,'Value',[])
  return
...
voltages = unique(getkv_bus(Bus,0,0));
