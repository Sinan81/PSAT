...
ncload = getnum_mn(Mn) + getnum_pl(Pl) +  getnum_lines(Lines);
if ncload || DAE.n
...
switch type
 ...
 case 2
  ...
  eta = Vmin(a);
 case 3
  a = findbus_pv(PV,bus_no);
  b = findbus_sw(SW,bus_no);
  if isempty(a) && isempty(b),
    fm_disp('No generator found for the specified bus number',2)
    failed = 1;
  end
  if a
    PQ = add_pq(PQ,pqdata_pv(PV,a,'qmaxl','',0,0,1));
    eta = getvg_pv(PV,a);
    PV = remove_pv(PV,a);
  end
  if b
    PQ = add_pq(PQ,pqdata_sw(SW,b,'qmaxl','',0,0,1));
    eta = getvg_sw(SW,b);
    SW = remove_sw(SW,b);
    SW = add_sw(SW,move2sw_pv(PV));
  end
 case 4
  a = findbus_pv(PV,bus_no);
  b = findbus_sw(SW,bus_no);
  ...
  if a
    PQ = add_pq(PQ,pqdata_pv(PV,a,'qminl','',0,0,1));
    eta = getvg_pv(PV,a);
    PV = remove_pv(PV,a);
  end
  if b
    PQ = add_pq(PQ,pqdata_sw(SW,b,'qminl','',0,0,1));
    eta = getvg_sw(SW,b);
    SW = remove_sw(SW,b);
    SW = add_sw(SW,move2sw_pv(PV));
  end
...
if Supply.n
  ...
  noSup = 1;
  if slack, SW = move2sup_sw(SW); end
end
...
while Settings.error > tol
  ...
  glambda_supply(Supply,lambda,slack*kg);
  Glcall_supply(Supply);
  if slack, Gkcall_supply(Supply); end

  if noSup
    glambda_pv(PV,lambda,slack*kg)
    glambda_sw(SW,lambda,kg)
    greactive_pv(PV)
    if slack, Gkcall_pv(PV), end
    Glcall_pv(PV)
    Gyreactive_pv(PV)
    Glcall_sw(SW)
  else
